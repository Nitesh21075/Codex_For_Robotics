...
This workspace starts empty on purpose. For the user's first request, create a concise `spec.json`, a README, and the code/assets needed to express the requested robot and task. Explain what was created. Installed PyBullet adapters support differential-drive, articulated hands/arms, and kinematic `rail_train` projects. For another morphology, first ask the RoboPilot MCP server for `get_adapter_capabilities`; select only an approved family, create the required project assets, create an adapter draft, and validate it. A draft is not runnable. Do not claim a simulation was run unless the RoboPilot tool returned a run record.
""",
}
